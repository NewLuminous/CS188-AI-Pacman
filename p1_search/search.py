...
def breadthFirstSearch(problem):
    ...
    if problem.isGoalState(StartPoint):
        return []

    Queue1 = util.Queue()
    visitedPoint = []
    Queue1.push((StartPoint, []))

    while not Queue1.isEmpty():
        currentPoint, Moves = Queue1.pop()
        if currentPoint not in visitedPoint:
            visitedPoint.append(currentPoint)

            if problem.isGoalState(currentPoint):
                return Moves

            for nextPoint, Move, cost in problem.getSuccessors(currentPoint):
                newMove = Moves + [Move]
                Queue1.push((nextPoint, newMove))
    ...
    while not pQueue.isEmpty():
        ...
        if currentNode not in visitedNodes:
            ...
            for nextNode, action, cost in problem.getSuccessors(currentNode):
                ...
                piority = prevCost + cost
                pQueue.push((nextNode, newAction, piority), piority)
